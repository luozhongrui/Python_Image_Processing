...
class getpulse(object):
    ...
    def get_rppg(self, title):
        ...
        for i, d in enumerate(dets):
            ...
            if(int(wFaceRight) < int(3 * wEye) and int(wFaceLeft) < int(3 * wEye)):
                ...
                self.roi = [int(self.fiter1.move_ave(cy - 0.5 * hNose - 22, 7)), int(self.fiter2.move_ave(cy - 0.5 * hNose - 10, 7)), int(self.fiter3.move_ave(cx - 6, 7)), int(self.fiter4.move_ave(cx + 6, 7))]

            if(int(wFaceLeft) > int(3 * wEye)):
                if self.left:
                    self.data_buffer, self.times = [], []
                    self.left = False
                    self.right = True
                ...
                self.roi = [int(self.fiter5.move_ave(shape.part(31).y - 6, 7)), int(self.fiter6.move_ave(shape.part(31).y + 6, 7)), int(self.fiter7.move_ave(shape.part(45).x - 6, 7)), int(self.fiter8.move_ave(shape.part(45).x + 6, 7))]

            if(int(wFaceRight) > int(3 * wEye)):
                if self.right:
                    self.data_buffer, self.times = [], []
                    self.right = False
                    self.top = True
                ...
                self.roi = [int(self.fiter9.move_ave(shape.part(31).y - 6, 7)), int(self.fiter10.move_ave(shape.part(31).y + 6, 7)), int(self.fiter11.move_ave(shape.part(36).x - 6, 7)), int(self.fiter12.move_ave(shape.part(36).x + 6, 7))]
            ...
            if L > 10:
                ...
                cv2.putText(self.frame_out, "fps:{}".format(self.fps), (10, 75), cv2.FONT_HERSHEY_PLAIN, 1.25, (0, 0, 255))
                ...
                cv2.putText(self.frame_out, text,
                            (10, 95), cv2.FONT_HERSHEY_PLAIN, tsize, col)
