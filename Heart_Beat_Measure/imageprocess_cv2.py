...
class getpulse(object):
    ...
    def get_subface_mean(self, rect, img):
        ...
        if len(sub_face[:, :]) < 1:
            print("get_subface_mean")
        val = np.mean(sub_face[:, :])
        return val
    ...
    def get_rppg(self, title):
        ...
        for i, d in enumerate(dets):
            ...
            if len(self.roi) != 0:
                cv2.rectangle(img, (self.roi[2], self.roi[0]), (self.roi[3], self.roi[1]), (0, 0, 255))
                ...
                if self.flag1:
                    self.data_buffer = self.data_buffer[-self.data_shift:]
                    self.times = self.times[-self.data_shift:]
                    self.flag1 = False
                    self.flag2 = False
                    self.flag3 = True
                ...
                if L > 10:
                    self.fps = int(float(L) / (self.times[-1] - self.times[0]))
                    cv2.putText(self.frame_out, "fps:{}".format(self.fps), (10, 95), cv2.FONT_HERSHEY_PLAIN, 1.5, (0, 0, 255))
                    ...
                    interpolated = (interpolated - np.mean(interpolated))/(np.std(interpolated)+10**-8)

                    # interpolated = (interpolated - np.mean(interpolated))

                    raws = np.fft.fft(interpolated)
                    raw = np.fft.fftshift(raws)
                    self.fft = np.abs(raw)
                    self.freqs = float(self.fps) / L * np.arange(L / 2 + 1)  # like k
                    freqs = 60. * self.freqs
                    idx = np.where((freqs > 40) & (freqs < 100))
                    # print(idx)
                    try:
                    ...
                    if gap:
                        if not self.flag1 and self.flag2:

                            text = "(rPpG: %0.1f bpm, wait %0.0f s)" % (self.bpm, gap)
                        else:
                            text = "(rPpG: %0.1f bpm)" % (self.bpm_s)
                    ...
                    cv2.putText(self.frame_out, text,
                                (10, 125), cv2.FONT_HERSHEY_PLAIN, tsize, col)
                else:
                    return
